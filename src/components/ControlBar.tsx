...
export default function ControlBar({ label, minLabel, maxLabel, value, isDraggable, size = 'large', onChange, name }: ControlBarProps) {
  const controlBarStyle =
    size === 'large'
      ? 'max-w-[720px] h-[28px] tablet:max-w-[624px] tablet:h-[26px]  mobile:max-w-[303px] mobile:h-[30px]'
      : size === 'medium'
  ...
  return (
    <div className={`mb-[18px] mobile:mb-[12px] ${controlBarStyle}`}>
      <div className='flex items-center justify-between'>
        <label className='mr-[16px] h-[28px] w-[56px] flex-none rounded-[6px] bg-gray-100 text-center text-md font-semibold leading-[28px] text-gray-500'>{label}</label>
        <span className='mr-[16px] w-[70px] flex-none whitespace-nowrap text-lg font-medium text-gray-800'>{minLabel}</span>
        ...
        <span className='ml-[16px] w-[70px] flex-none whitespace-nowrap text-right text-lg font-medium text-gray-800'>{maxLabel}</span>
      </div>
    </div>
  );
}
